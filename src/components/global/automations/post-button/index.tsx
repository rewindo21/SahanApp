...
// A React component for selecting and attaching Instagram posts to an automation. 
// It uses data from hooks to display user posts, tracks selected posts, and provides a button to confirm the attachment.
const PostButton = ({ id }: Props) => {
  // getting the user posts
  const { data } = useQueryAutomationPosts()
  // keep tracking what the user has selected
  const { posts, onSelectPost, mutate, isPending } = useAutomationPosts(id)

  return (
    <TriggerButton label="پیوست پست">
      {data?.status === 200 ? (
        <div className="flex flex-col gap-y-3 w-full">
          ...
          >
            <Loader state={isPending}>ضمیمه پست</Loader>
          </Button>
        </div>
      ) : (
        <p className="text-text-secondary text-center">پستی یافت نشد</p>
      )}
    </TriggerButton>
  )}
